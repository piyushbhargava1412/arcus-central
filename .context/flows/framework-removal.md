...
A user runs `arcus-integrate --remove` (or `integrate.sh --remove`) inside a target repository to undo the framework distribution. The script removes symlinks, copied agent/prompt/skill files, and leaves `.arcus/guidelines/` in place (preserving any copilot-instructions.md references).
...
arcus-integrate --remove
  └── integrate.sh (REMOVE_MODE=true, AUTO_YES=true)
        ...
        │     └── Preserve .arcus/guidelines/ (may be referenced by copilot-instructions.md)
...
- `.arcus/guidelines/` is intentionally preserved to avoid breaking existing `copilot-instructions.md` files in the target repo
